...
contract('ETHPool', (accounts) => {
    ...
    it('Team deposit reward but split fail because no users exists', async () => {
        ...
        // console.log(BigInt(balance));
        assert.equal(BigInt(balance), BigInt(web3.utils.toWei(String(1), 'ether')));
        // await ETHPoolInstance.withdraw({ from: user });
    });
    ...
    it('User A deposit, User B deposit, Team deposit reward', async () => {
        ...
        setTimeout(async () => {
            ...
            // console.log(BigInt(balance3));
            assert.equal(BigInt(balance3), BigInt(web3.utils.toWei(String(1), 'ether')));
